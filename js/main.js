...
function drop(ev) {
  ...
  if(ev.target.tagName === "IMG"){
    ...
    ev.target.closest("div").querySelector(".tierlistImgs").appendChild(document.getElementById(ev.dataTransfer.getData("text")));
  }
}
...
function removeLine(i){
  tierlistCross[i].addEventListener("click", function(){
    if(tierlistDiv[i].getElementsByTagName('img').length === 0){
      ...
      tierlistDiv[arrowUp.length - 1].parentNode.removeChild(tierlistDiv[arrowUp.length - 1])
    }
  })
}
...
function updateSelector(){
  ...
  tierlistDivTitleInput = Array.from(document.querySelectorAll(".tierlistDivTitleInput"))
